...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        Schema::create('documentos', function (Blueprint $table) {
            ...
            $table->string('cpf_arquivo')->nullable(); // caminho do arquivo CPF
            $table->string('rg_arquivo')->nullable(); // caminho do arquivo RG
            $table->string('comprovante_endereco')->nullable(); // comprovante de endereço

            // Certificação profissional (opcional)
            $table->string('certificado_profissional')->nullable();

            // Status de verificação de cada documento
            $table->enum('status_cpf', ['aguardando', 'aprovado', 'reprovado'])->default('aguardando');
            $table->enum('status_rg', ['aguardando', 'aprovado', 'reprovado'])->default('aguardando');
            $table->enum('status_comprovante', ['aguardando', 'aprovado', 'reprovado'])->default('aguardando');
            $table->enum('status_certificado', ['aguardando', 'aprovado', 'reprovado'])->default('aguardando');
