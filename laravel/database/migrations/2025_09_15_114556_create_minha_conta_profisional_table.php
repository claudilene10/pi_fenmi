...
return new class extends Migration
{
    ...
    {
        Schema::create('minhaContaProfissional', function (Blueprint $table) {
            $table->id('id_conta'); // PK
            $table->string('nome', 150); // Nome do usuário
            $table->string('profissao', 100)->nullable(); // Profissão
            $table->string('foto_perfil')->nullable(); // Caminho da foto de perfil
            $table->boolean('modo_escuro')->default(false); // Preferência de tema
            $table->boolean('validacao_documentos')->default(false); // Status da validação
            $table->boolean('emergencia_ativa')->default(false); // Botão de emergência
    ...
    {
        Schema::dropIfExists('minhaContaProfissional');
    }
};
