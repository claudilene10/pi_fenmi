...
return new class extends Migration
{
    ...
    {
        Schema::create('perfilProfissional', function (Blueprint $table) {
            $table->id('id_profissional'); // PK
            $table->string('nome', 150); // Nome do profissional
            $table->string('profissao', 100)->nullable(); // Profissão
            $table->string('foto_perfil')->nullable(); // Caminho da foto
            $table->text('sobre')->nullable(); // Texto da seção "Sobre"
            $table->json('servicos_oferecidos')->nullable(); // Lista de serviços (em JSON)
            $table->json('documentacao')->nullable(); // Documentos verificados
            $table->decimal('nota_media', 2, 1)->default(0.0); // Média das avaliações
            $table->integer('total_avaliacoes')->default(0); // Número de avaliações recebidas
    ...
    {
        Schema::dropIfExists('perfilProfissional');
    }
};
