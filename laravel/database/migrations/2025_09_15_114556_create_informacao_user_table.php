...
return new class extends Migration
{
    ...
    {
        Schema::create('infomacaoUser', function (Blueprint $table) {
            $table->id('id_informacao'); // Chave primária
            $table->string('nome', 150); // Nome do usuário
            $table->string('email', 150)->unique(); // E-mail único e obrigatório
            $table->string('telefone', 20)->nullable(); // Telefone opcional
            $table->string('cep', 9)->nullable(); // CEP opcional
            $table->string('endereco', 200)->nullable(); // Endereço opcional
    ...
    {
        Schema::dropIfExists('infomacaoUser');
    }
};
