...
return new class extends Migration
{
    ...
    {
        Schema::create('cadastroNormal', function (Blueprint $table) {
            $table->id('id_normal'); // PK
            $table->string('nome', 150); // nome completo
            $table->string('email', 254)->unique(); // email único
            $table->string('telefone', 20); // telefone com DDD
            $table->date('nascimento'); // data de nascimento
            $table->string('endereco', 255); // endereço completo
            $table->string('senha'); // senha (hash no backend)
            $table->enum('perfil', ['oferecer', 'contratar']); // tipo de perfil
             // Consentimentos
    ...
    {
        Schema::dropIfExists('cadastroNormal');
    }
};
