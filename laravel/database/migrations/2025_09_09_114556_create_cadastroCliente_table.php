...
return new class extends Migration
{
    ...
    {
        Schema::create('cadastroCliente', function (Blueprint $table) {
            $table->id('id_cliente'); // PK
            $table->string('nome', 150); // Nome completo
            $table->string('apelido', 100)->nullable(); // Apelido opcional
            $table->enum('Identidade_Gênero', ['Feminino', 'Masculino','Outros']); // Gênero
            $table->string('cpf', 14)->unique()->nullable(); // CPF (formato 000.000.000-00)
            $table->string('contato_emergencia', 20)->nullable(); // Contato de emergência
            $table->string('sms', 10)->nullable(); // Código de verificação por SMS
            $table->string('foto')->nullable(); // Caminho da foto de perfil
             // Consentimentos
    ...
    {
        Schema::dropIfExists('cadastroCliente');
    }
};
