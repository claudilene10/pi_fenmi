...
return new class extends Migration
{
    ...
    {
        Schema::create('historicoAtendimentos', function (Blueprint $table) {
            $table->id('id_atendimento');
            $table->foreignId('id_profissional')->constrained('users')->onDelete('cascade'); 
            $table->string('servico', 150);
            $table->string('cliente', 150);
            $table->string('local', 100)->nullable();
            $table->date('data');
            $table->enum('status', ['Concluído', 'Em andamento'])->default('Concluído');
            $table->time('tempo_execucao')->nullable(); // opcional (3h30, etc.)
            $table->tinyInteger('nota')->unsigned()->nullable(); // 1 a 5 estrelas
    ...
    {
        Schema::dropIfExists('historicoAtendimentos');
    }
};
