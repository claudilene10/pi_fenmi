...
class NovidadesController extends Controller
{
    ...
    /**
     * Recebe o ID do registro e busca no banco de dados retornando para a views
     */
    public function edit( Novidades $novidades )
    {  #Request $id = null

        #acessando diretamente o model
        #$item = \App\Modelos\Novidades::where('id_news','5'->first())

        #acessando usando a instancias da classe 
        # $item = $novidades -> where('id_news','5')-_first();


        # Recebe o paramâmetro id atravéz da Rota
        #busca os dados no banco
        $item = Novidades::where('id_news','5')->first();

        #manter os dados da tabela
        $todos = Novidades::all();

        # Retorna a view com os dados
        return view('novidades',[ 'edita ' => $item,'dados'=>$todos ]);
    }
    ...
    {
        #validar o dado antes da alteraçao e emotir erro
        $dadosValidados = $Request -> validate([
            'email' => 'required|email|max:254|unique:novidades,email',
        ]);
 
        $novidades -> update($dadosValidados);
     ...
     */
    public function destroy( $novidade)
    {
        #apaga o registro
        # Novidades::where('id_news',$novidade)->delete();
        Novidades::where('id_newa',$novidade)->delete();
        return retirect()->route(novidades);
    }
}
